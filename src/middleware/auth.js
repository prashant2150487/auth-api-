...
export const authorizeRoles = (allowedRoles = []) => {
  ...
  return async (req, res, next) => {
    try {
      ...
      if (
        !userRoleName ||
        (allowed.length > 0 && !allowed.includes(userRoleName))
      ) {
        return res
          .status(403)
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to authorize role" });
    }
  };
};
...
export const authorizePermissions = (requiredPermissions = []) => {
  ...
  return async (req, res, next) => {
    try {
      ...
      const rolePermissionIds = Array.isArray(user.role?.permissionIds)
        ? user.role.permissionIds
        : [];
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to authorize permissions" });
    }
  };
};
